...
class ReleaseNotesPlugin implements Plugin<Project> {
	...
	void apply(Project project) {
		...
		project.task('releaseNotes') {
			doLast {
				ReleaseNotes notes = prepareNotes(project)
		...
		project.task('progressReport') {
			doLast {
				ReleaseNotes notes = prepareNotes(project)

				def output = project.file(project.extensions.releaseNotes.progressReportFileName)
				println project.extensions.releaseNotes.progressReportFileName
				notes.generateProgressReport(output)
				println "Created label validation report at ${output.toURL().toString()}"
			}
		}
	}

	static ReleaseNotes prepareNotes(project) {
		def startHash = project.hasProperty('start') ? project.properties.get(
				'start') : project.extensions.releaseNotes.start
		def endHash = project.hasProperty('end') ? project.properties.get(
				'end') : project.extensions.releaseNotes.end
		def action = project.hasProperty('actionFilter') ? project.properties.get(
				'actionFilter').trim() : project.extensions.releaseNotes.actionFilter
		def label = project.hasProperty('label') ? project.properties.get(
				'label') : project.extensions.releaseNotes.label
		...
		return new ReleaseNotes(start: startHash,
								end: endHash,
